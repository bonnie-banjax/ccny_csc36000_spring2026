...
def distributed_compute(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def call_node(node: Dict[str, Any], sl: Tuple[int, int]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not resp.get("ok"):
                return {"error": f"node {node['node_id']} returned error: {resp}", "node_id": node["node_id"], "slice": list(sl)}
            ...
            return {"error": str(e), "node_id": node["node_id"], "slice": list(sl)}
    ...
    with ThreadPoolExecutor(max_workers=min(32, len(nodes_sorted))) as ex:
        ...
        for f in as_completed(futs):
            ...
            if res is None:
                continue
            if res.get("error"):
                print(f"[primary_node] Node {res.get('node_id')} failed slice {res.get('slice')}: {res.get('error')}")
                failed_slices.append(tuple(res.get("slice", [])))
            else:
                per_node_results.append(res)

    per_node_results.sort(key=lambda r: r["slice"][0])

    # If some slices failed on workers, compute them locally to ensure correctness
    if failed_slices:
        for sl in failed_slices:
            ...
            want_list = (mode == "list")
            try:
                local_res = get_primes(sl_low, sl_high, return_list=want_list)
            except Exception as e:
                print(f"[primary_node] local computation failed for slice {sl}: {e}")
                local_res = [] if want_list else 0
            t_local1 = time.perf_counter()
            if want_list:
                primes_list = list(local_res)
                total = len(primes_list)
                max_p = primes_list[-1] if primes_list else -1
            else:
                total = int(local_res)
                primes_list = []
                max_p = -1
            ...
            per_node_results.append({
                ...
                "node": {"host": "local", "port": 0, "cpu_count": 1},
                "slice": [sl_low, sl_high],
                "round_trip_s": t_local1 - t_local0,
                "node_elapsed_s": t_local1 - t_local0,
                "node_sum_chunk_s": 0.0,
                "total_primes": int(total),
                "max_prime": int(max_p),
                "primes": primes_list if want_list else None,
                "primes_truncated": False,
                "error": None,
            })
